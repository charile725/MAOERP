  ...
  const fetchKuji = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        // Load last prize
        if (kuji.last_prize_name || kuji.last_prize_product_id) {
          setHasLastPrize(true)
          // 使用 API 回傳的 last_prize_product 資料
          if (loadedSetType === 'custom' && kuji.last_prize_product_id && kuji.last_prize_product) {
            setLastPrize({
              name: kuji.last_prize_name || '',
              product_id: kuji.last_prize_product_id,
              product: kuji.last_prize_product as Product
            })
            setLastPrizeSearchInput(kuji.last_prize_product.name)
          } else {
            setLastPrize({
              name: kuji.last_prize_name || '',
              product_id: kuji.last_prize_product_id || '',
